...
1. Spectral has become a monorepo and CLI <-> Core have been split into separate packages. If you intend to use a CLI version of Spectral make sure to install `@api-commons/spotlight-cli`. Otherwise, go with `@api-commons/spotlight-core` which is the JS API.
...
```ts
import { Document, Spectral } from "@api-commons/spotlight-core";
import { Json as JsonParser } from "@api-commons/spotlight-parsers";
