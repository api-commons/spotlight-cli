...
1. Spectral has become a monorepo and CLI <-> Core have been split into separate packages. If you intend to use a CLI version of Spectral make sure to install `@spotlight-rules/spotlight-cli`. Otherwise, go with `@spotlight-rules/spotlight-core` which is the JS API.
...
```ts
import { Document, Spectral } from "@spotlight-rules/spotlight-core";
import { Json as JsonParser } from "@spotlight-rules/spotlight-parsers";
